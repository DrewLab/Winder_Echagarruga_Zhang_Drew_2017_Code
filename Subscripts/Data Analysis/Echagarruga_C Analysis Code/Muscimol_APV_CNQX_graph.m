function Muscimol_APV_CNQX_graph(~)
% close all;
clear all
for abc=1:2
    %open excl file to read in files
    [ndata, texter, alldata]=xlsread('Musc_APV_CNQX_Data.xlsx');
    s=size(alldata,1);
    s=s-1;
...
%% gaphing per animal_normalized
figure
bar(1,nanmean(y_normalized(2,1:8)./y_normalized(1,1:8)),'BarWidth',0.3,'FaceColor',[0.2 0.2 0.2])
hold on
scatter(ones(1,8),y_normalized(2,1:8)./y_normalized(1,1:8),'ko','MarkerFaceColor','w')
bar(2,nanmean(y_normalized(2,9:14)./y_normalized(1,9:14)),'BarWidth',0.3,'FaceColor',[0.4 0.4 0.4])
scatter(2*ones(1,6),y_normalized(2,9:14)./y_normalized(1,9:14),'ko','MarkerFaceColor','w')
bar(3,nanmean(y_normalized(2,15:18)./y_normalized(1,15:18)),'BarWidth',0.3,'FaceColor',[0.6 0.6 0.6])
scatter(3*ones(1,4),y_normalized(2,15:18)./y_normalized(1,15:18),'ko','MarkerFaceColor','w')
ylabel('\DeltaD/D \sigma_{musc.+CNQX+AP5} / \DeltaD/D \sigma_{aCSF}')
xlabel('Mouse #')
axis([0.5 3.5 0 2])
ax = gca;
set(ax,'XTick',1:3);
set(ax,'XTickLabel',1:3);
ylim([0 2])
