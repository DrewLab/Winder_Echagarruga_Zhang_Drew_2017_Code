function [AveR,Med_IndR] = EvaluateCBVPredictionAccuracy_Supplement(dataType1,dataType2,Beh,HRFs,CBVPred)
%   function [] =  ()
%
%   Author: Aaron Winder
...
elseif strcmp(Beh,'VW')
    Data1_end = 5; % Volitional whisks have maximum durations of 2 seconds, only consider the neural activity during this time
    strt = round((DataStruct1.epoch.offset-1)*DataStruct1.Fs); % Start using neural 0.3 seconds (30-45 frames) before the onset of event to allow for jitter in whisking detection
...
%     offset1 = mean(NormData1(test_inds,strt:stp),2)*ones(1,stp-strt+1);
...
%     offset1 = mean(NormData1(test_inds,strt:stp),2)*ones(1,stp-strt+1);
    ...
    Data1_end = 1.5; % Only consider neural activity within 1 second after the event as sensory evoked
...
%     offset2 = mean(NormData2(test_inds,strt:stp),2)*ones(1,stp-strt+1);
...
%     offset2 = mean(NormData2(test_inds,strt:stp),2)*ones(1,stp-strt+1);
    ...
    Data2_end = 3; % According to triggered average, CBV response has finished by 4 seconds after puff
...
if strcmp(Beh,'Rest')
    ...
    CCMat = corrcoef(mAct,mPred);
    AveR = CCMat(2,1);
end

% Calculate R-squared on individual data
IndR = NaN*ones(1,size(Data2,1));
if strcmp(Beh,'Rest')
    ...
    for tc = 1:length(Data2)
        ...
        mPred = Pred(strt:stp)-mean(Pred(strt:stp));
        mAct = Act(strt:stp)-mean(Act(strt:stp));
        CCMat = corrcoef(mPred,mAct);
        IndR(tc) = CCMat(2,1);
    end
    Med_IndR = median(IndR);
else
    if not(isfield(CBVPred,Beh))
