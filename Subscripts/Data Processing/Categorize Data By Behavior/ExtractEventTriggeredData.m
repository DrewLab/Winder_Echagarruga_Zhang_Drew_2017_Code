function [EventData] = ExtractEventTriggeredData(filenames,EventData,dataTypes,epoch)
%   [EventData] = ExtractEventTriggeredData(filenames,EventData,dataTypes,epoch)
%
%   Author: Aaron Winder
...
%   PARAMETERS:             
%                   filenames - [matrix] names of files organized as rows.
...
for f = 1:length(filenames)
    ...
    for dT = 1:length(dataTypes)
        ...
        for Bf = 1:length(Beh_fields)
            ...
            Data.TrialDur = StructData.TrialDur;
...
Fs = Data.Fs.([dataType '_fs']);

% Get the content from Data.(dataType)
DataVec = getfield(Data, {}, dataType, {});
...
chunk_data = zeros(sample_dur+1,length(epoch_starts),size(DataVec,1));
for st = 1:length(epoch_starts)
    chunk_inds = epoch_starts(st):epoch_stops(st);
    try
    chunk_data(:,st,:) = DataVec(:,chunk_inds)';
    catch
        breakpoint=1;
    end
end
