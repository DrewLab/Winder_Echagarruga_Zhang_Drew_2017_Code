function [Neuro, NeurFs] = ProcessNeuro(RawData,NeurType,animal,hem,numday)
...
Trimmed_Neuro = RawData.Data.Neuro(1:min(ExpectedLength,length(RawData.Data.Neuro)));
...
if ismember(NeurType,[{'MUpower'},{'Gam'},{'HiGam'},{'LoGam'},{'Beta'},{'SubAlpha'}])
    ...
    Neuro = max(resample(Long_Neuro,RawData.Fs.CBVCam,RawData.Fs.Analog),0);
    NeurFs = RawData.Fs.CBVCam;
    elapsed = toc;
...
elseif strcmp(NeurType,'Wideband_LFP')
    ...
    Neuro = resample(filtNeuro,NeurFs,RawData.Fs.Analog);
    elapsed = toc;
...
elseif nonzeros(ismember(NeurType,'SR'))
    ...
    Neuro = MUspikeRATE_gauss(MU_data,thresh,RawData.Fs.Analog,...
        Thresholds.(['MUA_StDev_' strday]));
    NeurFs = RawData.Fs.Analog;
    elapsed = toc;
end
